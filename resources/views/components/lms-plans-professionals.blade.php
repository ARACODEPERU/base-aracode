<div>

    <section id="planes" class="bg-dark section-dark text-light pt-80 relative" aria-label="section"
        style="background-size: cover; background-repeat: no-repeat;">
        <div class="container relative z-2" style="background-size: cover; background-repeat: no-repeat;">
            <div class="row g-4 justify-content-center" style="background-size: cover; background-repeat: no-repeat;">
                <div class="col-lg-6 text-center" style="background-size: cover; background-repeat: no-repeat;">
                    <div class="subtitle wow fadeInUp animated" data-wow-delay=".0s"
                        style="background-size: cover; background-repeat: no-repeat; visibility: visible; animation-delay: 0s; animation-name: fadeInUp;">
                        Una solución que se adapta a ti
                    </div>
                    <h2 class="wow fadeInUp animated" data-wow-delay=".2s"
                        style="visibility: visible; animation-delay: 0.2s; animation-name: fadeInUp;">
                        PLANES PROFESIONALES
                    </h2>
                    <p class="lead wow fadeInUp animated" data-wow-delay=".6s"
                        style="visibility: visible; animation-delay: 0.6s; animation-name: fadeInUp;">
                        Para instituciones pequeñas que inician <br> su transformación digital.
                    </p>
                </div>
            </div>
            <div class="row gy-4 gx-5 justify-content-center"
                style="background-size: cover; background-repeat: no-repeat;">
                <div class="col-lg-12" style="background-size: cover; background-repeat: no-repeat;">
                    <div class="row g-4" style="background-size: cover; background-repeat: no-repeat;">
                        <!-- ticket item begin -->
                        <div class="col-md-6" style="background-size: cover; background-repeat: no-repeat;">
                            <div class="relative overflow-hidden h-100 border-white-op-3 rounded-1 bg-blur"
                                style="background-size: cover; background-repeat: no-repeat;">
                                <div class="gradient-edge-bottom color op-5"
                                    style="background-size: cover; background-repeat: no-repeat;"></div>
                                <div class="p-40 pb-80 z-2"
                                    style="background-size: cover; background-repeat: no-repeat;">
                                    <div class="text-center"
                                        style="background-size: cover; background-repeat: no-repeat;">
                                        <h2 class="fs-40 mb-0">PROFESIONAL</h2>
                                        <h3 class="id-color mb-4">S/ 300.00 | Mensual</h3>
                                        <h4>S/ 3,000.00 | Anual</h4>
                                    </div>

                                    <div class="border-white-bottom-op-2 mb-4"
                                        style="background-size: cover; background-repeat: no-repeat;"></div>

                                    <ul class="ul-check mb-4">
                                        <li><strong>Dominio + SSL + hosting</strong></li>
                                        <li><strong>Pagina web (Estandar)</strong></li>
                                        <li>Pagina web contara con: Inicio, Nosotros, Cursos, Blog y Contacto.</li>
                                        <li>Carrito de compras: Tienda online para los cursos.</li>
                                        <li>Integración de pasarela de pagos: Mercado de Pago (Peru).</li>
                                        <li>Gestionar contenidos con sistema CMS</li>
                                        <li>Campus Virtual: Gestión de cursos, docentes, alumnos, matrículas,
                                            certificados automaticos</li>
                                        <li>Matrícula automática al comprar un cursos por la tienda en linea y matrícula
                                            manual a través del campus.</li>
                                        <li>Almacenamiento: 6 GB nativo + Integración de link de google drive para mayor
                                            almacenamiento</li>
                                        <li>4 usuario administrativo</li>
                                        <li>Hasta 500 alumnos activos</li>
                                        <li>4 correo corporativo</li>
                                        <li>Mantenimiento del sistema y Pagina web</li>
                                    </ul>
                                </div>

                                <div class="abs abs-center p-40 pb-30 bottom-0 z-2 w-100 text-center"
                                    style="background-size: cover; background-repeat: no-repeat;">
                                    <a class="btn-main fx-slide w-100" href=""><span>Lo Quiero</span></a>
                                </div>

                            </div>
                        </div>
                        <!-- ticket item end -->
                        <!-- ticket item begin -->
                        <div class="col-md-6" style="background-size: cover; background-repeat: no-repeat;">
                            <div class="relative overflow-hidden h-100 border-white-op-3 rounded-1 bg-blur"
                                style="background-size: cover; background-repeat: no-repeat;">
                                <div class="gradient-edge-bottom color op-5"
                                    style="background-size: cover; background-repeat: no-repeat;"></div>
                                <div class="p-40 pb-80 z-2"
                                    style="background-size: cover; background-repeat: no-repeat;">
                                    <div class="text-center"
                                        style="background-size: cover; background-repeat: no-repeat;">
                                        <h2 class="fs-40 mb-0">ACADÉMICO</h2>
                                        <h3 class="id-color mb-4">S/ 370.00 | Mensual</h3>
                                        <h4>S/ 3,700.00 | Anual</h4>
                                    </div>

                                    <div class="border-white-bottom-op-2 mb-4"
                                        style="background-size: cover; background-repeat: no-repeat;"></div>

                                    <ul class="ul-check mb-4">
                                        <li><strong>Dominio + SSL + hosting</strong></li>
                                        <li><strong>Pagina web (Personalizada)</strong></li>
                                        <li>Pagina web contara con: Inicio, Nosotros, Cursos, Servicios, Blog y Contacto.</li>
                                        <li>Carrito de compras: Tienda online para los cursos.</li>
                                        <li>Integración de pasarela de pagos: Mercado de Pago (Peru) y/o Paypal.</li>
                                        <li>Gestionar contenidos con sistema CMS</li>
                                        <li>Campus Virtual: Gestión de cursos, docentes, alumnos, matrículas,
                                            certificados automaticos</li>
                                        <li>Matrícula automática al comprar un cursos por la tienda en linea y matrícula
                                            manual a través del campus.</li>
                                        <li>Almacenamiento: 10 GB nativo + Integración de link de google drive para mayor
                                            almacenamiento</li>
                                        <li>6 usuario administrativo</li>
                                        <li>Hasta 600 alumnos activos</li>
                                        <li>5 correo corporativo</li>
                                        <li>Mantenimiento del sistema y Pagina web</li>
                                        <li>Actualizaciones progresivas</li>
                                    </ul>
                                </div>

                                <div class="abs abs-center p-40 pb-30 bottom-0 z-2 w-100 text-center"
                                    style="background-size: cover; background-repeat: no-repeat;">
                                    <a class="btn-main fx-slide w-100" href=""><span>Lo Quiero</span></a>
                                </div>

                            </div>
                        </div>
                        <!-- ticket item end -->
                    </div>
                </div>
            </div>
        </div>
    </section>
</div>
