<script setup>
    ...
    const form = useForm({
        ...
        next_payment_date: props.feeItem.next_payment_date,
        is_last_installment: props.feeItem.is_last_installment,
    });
    ...
    const addItem = (data) => {
        // 1 = servicio 2 = course
        let price = data.amount_to_pay;
        let is_product = false;
        let description = data.description;
        let xItemId = data.installment_id
        let unitType = 'ZZ';
    ...
    const sendSunatDocumentCreated = (document) => {
        Swal2.fire({
            ...
            customClass: 'sweet-alerts',
            preConfirm: () => {
                return axios.get(route('saledocuments_send', [document.id,document.invoice_type_doc]), { timeout: 30000 }).then((res) => {
                    if (!res.data.success) {
...
<template>
    ...
    <div v-if="message" class="">
        <div id="content">
            ...
            <h1 class="block text-7xl font-bold text-gray-800 sm:text-9xl dark:text-white">204</h1>
            <p class="mt-3 text-gray-600 dark:text-neutral-400">Oops, No hay cuotas pendientes.</p>
            <p class="text-gray-600 dark:text-neutral-400">La venta seleccionada no tiene cuotas pendientes por pagar..</p>
            <div class="mt-5 flex flex-col justify-center items-center gap-2 sm:flex-row sm:gap-3">
                <a @click="closeMyPopup" class="w-full sm:w-auto py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-hidden focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none" href="../examples.html">
                    Cerrar ventana
                </a>
            </div>
            </div>
                ...
                <div class="mt-8 px-4">
                    <div class="grid sm:grid-cols-2 grid-cols-1 gap-4 justify-between relative">
                        ...
                        <div>
                            ...
                            <div>
                                <p>Cuotas pagadas: {{ feeItem.installment_number }} / {{ feeItem.total_installments }}</p>

                                <p v-if="feeItem.is_last_installment">
                                    Esta es la última cuota. No se realizarán más pagos.
                                </p>

                                <div v-else>
                                    <label>FECHA PRÓXIMO PAGO</label>
                                    <input v-model="form.next_payment_date" type="date" class="form-input max-w-[160px]" />
                                </div>
                            </div>
                        </div>
