...
class CommercialNegotiationProcessController extends Controller
{
    ...
    public function processWebhook(Request $request, $id)
    {
        ...
        try {
            ...
            // El paso "Finalizar" corre despues de este y es el que marcaba al aprobador,
            // asi que sin esto el JSON viajaba con aprobado_por en null.
            $this->registerApprover($negotiation);

            $payload = NegotiationWebhookPayload::forNegotiation($negotiation, $person);
    ...
    /**
     * Deja constancia de quien aprobo la negociacion. Si ya se aprobo antes desde
     * el detalle (boton "Aprobar"), se respeta ese registro y no se pisa.
     */
    private function registerApprover(CommercialNegotiation $negotiation): void
    {
        if ($negotiation->verified_by) {
            return;
        }
