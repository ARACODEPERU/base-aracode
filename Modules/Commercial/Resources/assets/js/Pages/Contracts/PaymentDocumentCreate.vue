<script setup>
 import AppLayout from '@/Layouts/Vristo/AppLayout.vue';
    ...
    import SearchProducts from 'Modules/Sales/Resources/assets/js/Pages/Documents/Partials/SearchProducts.vue';
    ...
    const defaultSaleDocumentType = props.commercialPayment?.client?.document_type_id == 6 || props.client?.document_type_id == 6 ? 1 : 2;
    ...
    const formDocument = useForm({
        ...
        client_ubigeo_description: props.commercialPayment ? (props.client.city || props.client.ubigeo_description) : props.company.city,
        client_direction: props.commercialPayment ? props.client.address : props.company.fiscal_address,
        client_phone: props.client.telephone,
        client_email: props.client.email,
        sale_documenttype_id: defaultSaleDocumentType,
        ...
        commercial_payment_id: props.commercialPayment?.id,
        payments: [{
            type:1,
            reference: null,
            amount: Number(props.commercialPayment?.balance_amount || props.commercialPayment?.total_amount || 0).toFixed(2)
        }],
    ...
    const setCommercialPaymentData = () => {
        if (!props.commercialPayment) return;

        const payment = props.commercialPayment;
        const contract = payment.contract || {};
        const service = contract.service || {};
        const amount = Number(payment.balance_amount || payment.total_amount || payment.amount || 0);
        const description = payment.description || `Cuota ${payment.payment_number}`;

        formDocument.date_end = payment.due_date || formDocument.date_end;
        //formDocument.additional_description = `Contrato: ${contract.title || '-'} | ${description}`;

        getDataTable({
            id: service.id || null,
            interne: service.interne || null,
            //description: `${service.description || 'Servicio'} - ${description}`,
            description: `${service.description || 'Servicio'}`,
            is_product: service.is_product == 1,
            unit_type: service.type_unit_measure_id || 'ZZ',
            quantity: 1,
            unit_price: amount.toFixed(2),
            discount: 0,
            total: 0,
            afe_igv: service.type_sale_affectation_id || '10',
            size: null,
            m_igv: 0,
            presentations: service.presentations || null,
            icbper: service.icbper || false,
        });
    };
    ...
    const saveDocument = () => {
        ...
        if(formDocument.serie){
            ...
            axios.post(route('comm_contract_payment_document_store'), formDocument ).then((res) => {
                console.log(res)
        ...
        Swal2.fire({
            ...
            if (result.isConfirmed) {
                ...
                Swal2.fire({
                    ...
                    router.visit(route('comm_contracts_payments', commercialPayment.contract.id),{
                        method: 'get'
                    });
                });
...
<template>
    <AppLayout title="Documento de venta">
        <Navigation :routeModule="route('comm_dashboard')" titleModule="Comercial"
            :data="[
                {route: route('comm_contracts'), title: 'Contratos'},
                {route: route('comm_contracts_payments', commercialPayment.contract.id), title: 'Cronograma'},
                ...
                <div class="panel">
                                ...
                                <tr class="bg-gray-100 dark:bg-gray-900 dark:bg-opacity-20">
                                    <th style="width: 60px;" class="text-right py-1">
                                        <div class="flex items-center justify-center gap-2">
                                            <button v-can="'sale_registar_producto_alvender'" @click="newItems" type="button" v-tippy="{ content: 'Agregar nuevo', placement: 'bottom'}" class="btn btn-sm btn-outline-success">
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        ...
                        <div class="col-span-4 sm:col-span-1 space-y-2">
                            ...
                            <Link :href="route('comm_contracts_payments', commercialPayment.contract.id)" class="btn btn-danger uppercase text-xs w-full">
                                Cancelar
                            </Link>
                        </div>
                    </div>
